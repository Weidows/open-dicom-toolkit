...
## XI：示例 Operator 模板
````python
# name=operators/template_operator.py
class TemplateOperator(OperatorBase):
    name = "template"
    version = "0.1"
    ...
    def run(self, ctx: dict) -> dict:
        img = ctx["image"]
        # ... do work ...
        mask = (img > 0.5).astype("uint8")
        ctx["mask"] = mask
        return ctx
````

## XII：Roadmap（优先级）
1. 定义并实现 OperatorBase + Registry（必要的元数据与测试用例）。
2. 实现 DICOM Reader、MetaExtractor、USPreprocess、ModelOperator、MeasurementOperator、ReportGenerator 的样板代码（带单元测试）。
3. 提供简单 LLM Planner 示例：把自然语言转成 workflow YAML（本地 rule-based + demo LLM 模型）。
4. 打包若干示例模型（ONNX）与 Docker plugin 模板；集�� Orthanc。
5. 提供 end-to-end demo：从 DICOM 文件夹到 DICOM SR 输出并支持人工复核。

## XIII：下一步交付（我可以帮你生成）
- [ ] Operator SDK 模板（Python package + entrypoint +示例单元测试）。
- [ ] MONAI Deploy / ONNX Runtime 推理 Operator 的示例实现（含 Dockerfile）。
- [ ] LLM Planner → Workflow 示例（含 prompt 模板 + 执行器 reference）。
- [ ] 一个可运行的 demo repo（docker-compose: Orthanc + API server + 1 个模型 plugin）。
